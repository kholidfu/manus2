...
def download(title):
    ...
    return redirect("http://ads.ad-center.com/offer?prod=141&ref=4988911&q=" + title.replace("-", " ").title())
...
def sitemap_files_depth_2(a, b):
    ...
    dir = []
    for i in os.listdir(basedir):
        dir.append("%s/%s/%s" % (a, b, i))
    sitemap = render_template("sitemap_details.xml", data=dir)
    response = make_response(sitemap)
    response.headers['Content-Type'] = 'application/xml'
    return response
